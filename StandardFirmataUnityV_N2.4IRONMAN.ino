...
#define MAX_HEATER_ON_TIME_IN_SECONDS 5400 //90 minutes = 5400 sec

//
#define heatPin1 7 //control on/off of heaters. If pin is low, will turn heater on
#define heatPin2 8 //control on/off of heaters. If pin is low, will turn heater on
#define fanPin 11

#define BurstControlPin 4
//#define LEDPin 13
#define motorPin 3

#define limSwitchPin 12
...
unsigned int samplingInterval2 = 0; // how often to run the main loop (in ms)
//
...
void stringCallback(char *myString) {
  String burst_str =  String(myString);  
  
  int num_bursts = burst_str.toInt(); // parseInt(myString[0]);
  fireBurst(num_bursts);
}

void fireBurst(int num_bursts) {
  int actualcount = 0;
  int count = 0;

  digitalWrite(heatPin1,LOW);
  while (num_bursts > actualcount) { //this part of the code keeps count and compares to te countOTT
      analogWrite(motorPin,130);
      analogWrite(bluePin,0);
      analogWrite(redPin,20);
      
      int limit_pin = digitalRead(limSwitchPin); 
      
      if (limit_pin == LOW){
         count = count+1;
         delay(100);
      } else if (count > 0 && limit_pin == HIGH) {
        count = 0;
...
//TCCR1B = (TCCR1B & 0b11111000) | 0x01;
  ...
  //
   TCCR2B = (TCCR2B & 0b11111000) | 0x03; //Increase PWM frequency to eliminate motor buzzing
  //
...
int ReadBurstControlPinState()
{
  state = bitRead(PORTD,BurstControlPin);
  return state;
  //Serial.println(state);
}


void ReadLimitSwitchWithDelayBurst()
{
...
if(n_rev<=0)
{
 ...
 while (n_rev > actualcount) 
 {
      ...
      //this part of the code keeps count and compares to te count
      analogWrite(motorPin,120);
       // analogWrite(motorPin,200);
      
      
      int switch1 = digitalRead(limSwitchPin); 
      //define the limit switch as switch 1
...
void ReadSerialPort()
{
   ...
   while(dataReading<'a' || dataReading>'z')
   {
      
      delay(2);
      dataReading = Serial.read();
      //Serial.println(inData);
   }
   
   if(dataReading>='a' && dataReading<='z')
    {
       //Serial.println((char)dataReading);
        n_rev = (dataReading - (int)'a')+1;
    } 
...
prevState = state;
  ...
  if ((heatOnTimer != 0) && ((millis() - heatOnTimer)/1000) > MAX_HEATER_ON_TIME_IN_SECONDS) {
    digitalWrite(heatPin1,LOW); // turn off heater
    heatOnTimer = 0; // reset timer
  }
    ...
    for (pin = 0; pin < TOTAL_PINS; pin++) {
      if (IS_PIN_ANALOG(pin) && pinConfig[pin] == PIN_MODE_ANALOG) {
        ...
        if (analogInputsToReport & (1 << analogPin)) {
          if(analogPin==BurstControlPin){
           //Firmata.sendAnalog(BurstReaderPin, limitSwitchCounter); 
